...
switch($e){
    ...
    case 'property':
        ...
        'main_description', 'images_count','price_rent','price_rent_computed','price_sell_currency', 'price_sell', 'prop_state_id', 'realestate',
        'main_image_url'];
        break;
    case 'images':
        $e = 'ImageFile';
...
// del tipo /propiedades/99
/*
$query = $datastore->gqlQuery("SELECT * FROM Property WHERE destacada = @dest AND realestate = @reid", [
    'bindings' => [
        'dest' => 1,
        'reid' => $task['realestate']
    ]
]);
*/
/*
...
$query->kind('Property');
//$query->filter('prop_operation_id', '=', '2');
$query->filter('destacada', '=', 1);
*/


/*
$query = $datastore->gqlQuery( "SELECT * FROM PropertyIndex WHERE price_rent_computed>null ORDER BY published_at DESC LIMIT @li ", [
    'bindings' => [
...
$query = $datastore->query()
->kind('Property')
->order('updated_at', Query::ORDER_DESCENDING)
->limit(30);
...
foreach ($result as $index => $task) {
    ...
    $response[$att['prop_operation_id']][] = $data;
